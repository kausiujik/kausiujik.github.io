...
其中$j$时满足$\max\{f_1',f_2',\dots,f_n'\}<1$的最小整数.
...
$${\rm CAIM}=\dfrac{1}{N}\sum\limits_{j=1}^k\dfrac{M_j^2}{n_{\cdot j}},$$
...
$${\rm rd}_k(x_1,x_2)=\max\{d_k(x_2),d(x_1,x_2)\}.$$

基于可达距离, 进一步定义**局部可达密度(local reachability density)**, 它是样本与其近邻样本的平均可达距离的倒数:

$${\rm lrd}_k(x)=\left(\dfrac{1}{k}\sum\limits_{y\in N_k(x)}{\rm rd}_k(x,y)\right)^{-1}.$$

利用样本及其近邻样本的局部可达密度, 可以计算样本的局部离群因子:

$${\rm lof}_k(x)=\dfrac{1}{k}\sum\limits_{y\in N_k(x)}\dfrac{{\rm lrd}_k(y)}{{\rm lrd}_k(x)}.$$

进一步我们可以根据以下规则对样本是否离群做判断: 如果${\rm lof}_k(x)$接近于1, 代表样本$x$与其近邻样本的局部可达密度相似, 可以认为是正常样本; 如果${\rm lof}_k(x)<1$, 代表样本$x$的局部可达密度大于其近邻样本, 可以认为是正常样本; 如果${\rm lof}_k(x)>1$, 代表样本$x$的局部可达密度小于其近邻样本, 可能为离群值.
