...
进一步，还可以得到 $\mathbb E_f[h(X)]$ 的渐近水平为 $1-\alpha$ 的置信区间 $\left[\overline{h}_m\pm u_{\alpha/2}s_m/\sqrt{m}\right]$。
...
其中 $g_1$ 为形状参数为 $n/2+1$、尺度参数为 $n[(\theta-\overline{x})^2+s_n^2]/2$ 的逆伽马密度函数，$g_2$ 为自由度为 $n+1$、位置参数为 $\overline{x}$、尺度参数为 $s_n/\sqrt{n+1}$ 的 $t$ 分布密度函数。
