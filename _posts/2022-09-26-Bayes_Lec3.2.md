...
因此可见，${\rm BF}_{01}(\pmb x)$ 正是 $H_0\leftrightarrow H_1$ 的似然比，它通常被认为是由数据给出的 $H_0\leftrightarrow H_1$ 的优势比。由于这种情形的贝叶斯因子不依赖于先验分布，仅依赖于样本的似然比，故贝叶斯因子 ${\rm BF}_{01}(\pmb x)$ 可视为是数据 $\pmb x$ 支持 $H_0$ 的程度。
