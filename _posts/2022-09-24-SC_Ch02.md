...
---
**命题.** 记 $\displaystyle\hat{\theta}^M=\frac{1}{M}\sum_{i=1}^Mg(U_i)$ 为直接采用 $M$ 个样本做的简单MC估计，$\displaystyle\hat{\theta}^S=\frac{1}{k}\sum_{j=1}^k\hat{\theta}_j$ 为分层估计量（这里假设均匀分层，即 $m=M/k$），其中 $\displaystyle\hat{\theta}_j=\frac{1}{m}\sum_{i=1}^mg(U_{ji})$。于是
<center>
