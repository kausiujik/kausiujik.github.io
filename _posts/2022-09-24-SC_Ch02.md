...
<center>
$\text{Var}\left(\dfrac{U_1+U_2}{2}\right)=\dfrac{\text{Var}(U_1)+\text{Var}(U_2)+{\rm Cov}(U_1,U_2)}{4}<\dfrac{\text{Var}(U_1)+\text{Var}(U_2)}{4}.$
</center>
...
<center>
$\displaystyle c^\star=-\dfrac{{\rm Cov}(g(X),f(X))}{\text{Var}(f(X))},$
</center>
...
<center>
$\displaystyle\mathbb E[f(U)]=0.5,\quad c^\star=-\frac{{\rm Cov}({\rm e}^U,U)}{\text{Var}(U)}=6{\rm e}-18.$
</center>
...
其中每一层的采用MC估计 $\hat{\theta}_i$。不难证明 $\text{Var}(\hat{\theta}^S)\leq\text{Var}(\hat{\theta}^M)$，这里 $\hat{\theta}^M$ 是直接采用 $m$ 个样本做的简单MC估计。
