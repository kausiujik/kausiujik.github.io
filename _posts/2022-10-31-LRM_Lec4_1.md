...
其中 $r_{ij}=\dfrac{\sum\limits_{k=1}^n(x_{ki}-\overline{x}_i)(x_{kj}-\overline{x}_j)}{s_is_j}$，$i,j=1,\dots,p$ 为自变量 $x_i$ 与 $x_j$ 之间的样本相关系数。所以 $\pmb R$ 是自变量的样本相关系数矩阵。经过标准化后的线性回归模型为
<center>
...
特别地，取 $\pmb c=(0,\dots,0,1,0,\dots,0)'$，其中 $1$ 在 $\pmb c$ 的第 $i+1$ 个位置，则 $\pmb c'\pmb\beta=\beta_i$，$\pmb c'\hat{\pmb\beta}=\hat{\beta}_i$。记 $(\pmb A)_{ii}$ 表示矩阵 $\pmb A$ 的第 $(i,i)$ 元素，则
<center>
