...
其中 $\left(\dfrac{\partial u}{\partial\pmb Y}\right)_{ij}$ 表示矩阵 $\dfrac{\partial u}{\partial\pmb Y}$ 的第 $(i,j)$ 元，$(\pmb Y)_{ij}$ 表示矩阵 $\pmb Y$ 的第 $(i,j)$ 元。
...
- 对任意矩阵 $\pmb A,\pmb X,\pmb B$，
    ...
    这里用到了 $\dfrac{\partial(\pmb A\pmb X\pmb B)_{ij}}{\partial\pmb X}=\pmb A'\pmb E_{ij}\pmb B'$。
